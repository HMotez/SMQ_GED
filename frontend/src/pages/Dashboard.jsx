          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <GlassCard className="p-6">
              ...
              {loadingOv ? <p className="text-sm text-center py-8" style={{ color:"rgba(168,191,212,0.5)" }}>Chargement…</p>
                ...
                : <div className="flex flex-col gap-2 max-h-60 overflow-y-auto">{expired.list.map(doc => <AlertRow key={doc.id} doc={doc} accent="#f87171" onClick={() => navigate("/list")} />)}</div>
              }
            ...
            <GlassCard className="p-6">
              ...
              {loadingOv ? <p className="text-sm text-center py-8" style={{ color:"rgba(168,191,212,0.5)" }}>Chargement…</p>
                ...
                : <div className="flex flex-col gap-2 max-h-60 overflow-y-auto">{inValidation.list.map(doc => <AlertRow key={doc.id} doc={doc} accent="#a5b4fc" onClick={() => navigate("/validations")} />)}</div>
              }
          ...
          {overdue.list.length > 0 && (
            <div className="rounded-2xl border p-6" style={{ background:"rgba(251,146,60,0.06)", borderColor:"rgba(251,146,60,0.2)" }}>
              ...
              <div className="grid gap-2" style={{ gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))" }}>
                {overdue.list.map(doc => <AlertRow key={doc.id} doc={doc} accent="#fb923c" onClick={() => navigate("/list")} />)}
              </div>
            </div>
          )}
        </div>
