    ...
    >
      ...
      <div className="max-w-[1280px] mx-auto px-8 h-16 flex items-center justify-between">
        ...
        <NavLink to="/" className="no-underline flex items-center gap-3">
          <div
            className="flex gap-0.5 p-2 rounded-xl border"
            style={{ background: "rgba(74,184,63,0.12)", borderColor: "rgba(74,184,63,0.25)" }}
          >
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex flex-col gap-0.5">
                {[...Array(3)].map((_, j) => (
                  <div
                    key={j}
                    className="dot-float w-1.5 h-1.5 rounded-sm"
                    style={{
                      background: "#4ab83f",
